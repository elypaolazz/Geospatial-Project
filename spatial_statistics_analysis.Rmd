...
This notebook aims to perform a spatial statistics analysis concerning ski pass prices in the area (the Dolomite area in the province of Bolzano), investigating whether there are any relationships between them.
Since we are unable to find a complete resource containing the costs of the ski pass for the Dolomiti area, the data used in this notebook was created in the file xx.ipynb by consulting the price lists on the websites of the various resorts.
...
#### Which ski area is the less expensive and where are it?
...
plot(dnb199, BZ_dolomiti_ski_areas, add = TRUE, col = "yellow")
dnb199
...
plot(area_dolomiti_BZ, border = "grey", xlab = "", ylab = "", xlim = NULL)
title(main = "d nearest neighbours, d = 0.1991") 
plot(dnb199, BZ_dolomiti_ski_areas, add = TRUE, col = "orange")
dnb199
...
The test was performed under the assumption of normality since radomisation parameter is set on False. Let's try under the assumption of randomization:
```{r}
...
The values are only slightly different according to the 3 approaches used (especially between normaliti and radomization assumptions). When more neighbours are considered (both in the knn and in the critical cut-off) there seems to be a very small positive global spatial autocorrelation. In the most significant case of the knn, with 6 and 8 neighbours and in the case of the highest distances with the critical cut-off, it seems to reach at most a positive correlation of 0.12. This is actually very low and makes us think that there is no effective global correlation.
